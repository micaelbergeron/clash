...
export default class ActionList extends React.Component {
  ...
  render() {
    const enable = ({enable, disable}) =>
      (!enable && !disable) || (!disable && enable.call(this, this.props))
                            || (!enable && disable.call(this, this.props))

    const ActionListItem = (action, k) => {
      const enabled = enable(action)
      if (enabled) {
        return (
          <div tabIndex="0" className="menu__item" key={k} onClick={(e) => action.action.call(this, e, this.props)}>
    ...
    let handlers = R.map(a => (event) => a.action.call(this, event, this.props), actions)
